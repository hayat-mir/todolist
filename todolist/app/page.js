...
const page = () => {
  ...
  if (mainTask.length > 0) {
    renderTask = mainTask.map((t, i) => {
      return (
        <li key={i} className="flex items-center justify-between mb-8 px-10 ">
          <div className="flex items-center justify-between mb-5 w-2/3">
            <h5 className="text-2xl font-semibold">{t.title}</h5>
            <h6 className="text-xl font-semi-bold">{t.desc}</h6>
          </div>
          <button
            onClick={() => {
              deleteHandler(i);
  ...
  return (
    <>
      ...
      <form onSubmit={submitHandler}>
        ...
        />
        <button className="bg-black text-white m-5 px-4 py-3 text-2xl font-bold rounded">
          Add Task
        </button>
      </form>
      <hr />
